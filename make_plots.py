...
def plot_q_dist(DM,SIDM,ax,method=''):
    ...
    eq1h_dm = np.percentile(DM.mcmc_q1h_gt[1500:], [16, 84])
    eq2h_dm = np.percentile(DM.mcmc_q2h_gt[1500:], [16, 84])
    eq1h_sidm = np.percentile(SIDM.mcmc_q1h_gt[1500:], [16, 84])
    eq2h_sidm = np.percentile(SIDM.mcmc_q2h_gt[1500:], [16, 84])
...
def linear_compare_q(DM,SIDM,ax,j):    
    ...
    eq_1h_dm = np.diff(np.percentile(DM.mcmc_q1h_gt[1500:], [16, 50, 84]))
    eq_2h_dm = np.diff(np.percentile(DM.mcmc_q2h_gt[1500:], [16, 50, 84]))
    eq_1h_sidm = np.diff(np.percentile(DM.mcmc_q1h_gt[1500:], [16, 50, 84]))
    eq_2h_sidm = np.diff(np.percentile(DM.mcmc_q2h_gt[1500:], [16, 50, 84]))
...
def compare_q(DM,SIDM,ax,j):    
    ...
    ax1.errorbar(j+0.,(DM.q1h_gt/np.mean(DM.q2d_it))-1,
                 yerr=np.array([eq_1h_dm]).T,
                 fmt='ks',markersize=10,label='CDM - standard')
    ax1.errorbar(j+0.1,(DM.q1h_gt/np.mean(DM.q2dr_it))-1,
                 yerr=np.array([eq_1h_dm]).T,
                 fmt='C7D',markersize=10,label='CDM - reduced')
    ax1.errorbar(j+0.2,(SIDM.q1h_gt/np.mean(SIDM.q2d_it))-1,
                 yerr=np.array([eq_1h_dm]).T,
                 fmt='C3s',markersize=10,label='SIDM - standard')
    ax1.errorbar(j+0.3,(SIDM.q1h_gt/np.mean(SIDM.q2dr_it))-1,
                 yerr=np.array([eq_1h_dm]).T,
                 fmt='C6D',markersize=10,label='SIDM - reduced')
    ...
    ax2.errorbar(j+0.2,(DM.q1h_gt/SIDM.q1h_gt)-1,
                 yerr=e_ratio_1h,
                 fmt='C1o',markersize=10,label='$q_{1h}$')
    ax2.errorbar(j+0.3,(DM.q2h_gt/SIDM.q2h_gt)-1,
                 yerr=e_ratio_2h,
